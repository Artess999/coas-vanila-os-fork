...
void CaptureMerchant(string temp)
{
    ...
    if (trap)
    {
        ...
        string stext1 = "Прошел слух, что лихой пират - "+GetFullName(pchar)+", угодил в западню, в которую его заманили "+NationNameSK(sti(MerPrm.ination))+"ие власти. Они подослали к нему ''торговое'' судно, в трюме которого, спрятались вооруженные до зубов "+NationNameSK(sti(MerPrm.ination))+"ие  солдаты. Но "+pchar.name+" не растерялся, он их попросту перебил - всех до единого! Вот какой удалой пират наш "+GetFullName(pchar)+".";
        string stext2 = "Люди говорят, что дерзкий пират - "+GetFullName(pchar)+", повадился грабить "+NationNameSK(sti(MerPrm.ination))+"ие торговые корабли. И тогда наш Генерал-губернатор решил покончить с ним. Он снаряди  корабль, замаскированный под торговца с несколькими десятками хорошо обученных солдат, из Европы, и подослал к нему. Но кровожадный пират перебил их всех и вновь улизнул от правосудия!";
