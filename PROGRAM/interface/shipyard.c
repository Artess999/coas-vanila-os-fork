...
void ClickRepairArror(string _type, int add)
{
	...
	if (_type == "hull")
	{
		if (add > 0)
		{
			if (sti(pchar.Money) >= (GetSailRepairCost(st, RepairSail) + GetHullRepairCost(st, RepairHull + add)))
		    {
		        RepairHull = RepairHull + add;
		    }
		    else
		    {
		        i = sti(pchar.Money) - (GetSailRepairCost(st, RepairSail) + GetHullRepairCost(st, RepairHull));
		        i = i / GetHullRepairCost(st, 1); // на сколько хватит
	...
	else
	{
	    if (add > 0)
		{
			if (sti(pchar.Money) >= (GetSailRepairCost(st, RepairSail+ add) + GetHullRepairCost(st, RepairHull)))
		    {
		    ...
		    {
		        i = sti(pchar.Money) - (GetSailRepairCost(st, RepairSail) + GetHullRepairCost(st, RepairHull));
		        i = i / GetSailRepairCost(st, 1); // на сколько хватит
...
void RepairOk()
{
	...
	if (RepairHull > 0)
	{
		...
		AddMoneyToCharacter(pchar, -GetHullRepairCost(st, RepairHull));
	...
	if (RepairSail > 0)
	{
		...
		AddMoneyToCharacter(pchar,-GetSailRepairCost(st, RepairSail));
