...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "CaracasMan":
			dialog.text = "Эй, моряк, тоже хочешь поучаствовать? Лишним не будешь!";
		...
		case "CaracasWervolf_2":
			...
			link.l1 = "Я погубил столько душ, что помощь в спасении хотя бы одной, пожалуй, мне зачтется. Ждите здесь, постарайтесь продержаться!";
			link.l1.go = "exit";
			npchar.lifeDay = 0;
		...
		break;
		case "ShoreGuard_1":
			dialog.text = "Как бы ты сам не докукарекался, петушок! Не повернешь назад добром - заставим!";
		...
		case "DMCitiz":
			...
			{
				dialog.text = LinkRandPhrase("Будь ты проклят! Уходи, пока еще можешь это сделать...", "Давай, беги отсюда, трус несчастный...", "Вали отсюда! И чтобы я тебя здесь больше не видел...");
				link.l1 = "Обязательно!";
				link.l1.go = "exit";
			}
			if (pchar.questTemp.MC == "toSeekSister" || pchar.questTemp.MC == "toSeekSkull" || pchar.questTemp.MC == "toSeekQueen")
			{
				dialog.text = LinkRandPhrase("Говорят, что вы согласились помочь нам. Удачи вам, капитан...", "Да благословит вас Господь, сеньор капитан!", "Как я желаю, чтобы у вас все получилось!");
				link.l1 = "Обязательно!";
				link.l1.go = "exit";
			}
			if (pchar.questTemp.MC == "over")
			{
				dialog.text = "Сеньор, вы наш спаситель! Да хранит вас Господь!";
				link.l1 = "Спасибо. Тронут, тронут...";
				link.l1.go = "exit";
			}
		break;
		...
		case "DMCitizHead":
			...
			{
				dialog.text = "Сеньор капитан, как только узнаете что-то новое - сразу прошу уведомлять меня. По сути, вы - наша последняя надежда на нормальную жизнь.";
			...
			if (pchar.questTemp.MC == "toSeekSkull")
			{
				...
				link.l1 = "Я нашел Элизабет Шеппард.";
				link.l1.go = "FoundLiz";
			}
			if (pchar.questTemp.MC == "toSeekQueen")
			{
				dialog.text = "Здравствуйте, капитан! Что скажете?";
				link.l1 = "Я отравился на поиски брига 'Королева'. Похоже, что это последний шаг к вашему освобождению.";
				link.l1.go = "exit";
			}
			if (pchar.questTemp.MC == "over")
			{
				dialog.text = "Это вы сняли проклятие? В какой-то момент у нас все нормализовалось!";
				link.l1 = "Да. Я нашел бриг 'Королева' и все сделал. Рад, что сумел вам помочь.";
				link.l1.go = "DMHeadOver";
			}
			if (CheckAttribute(pchar, "questTemp.MC.DesMoines"))
			{
				dialog.text = "Сеньор, наш спаситель! Вы что-то хотели?";
		...
		case "DMCitizHead_1":
			...
			link.l1 = "Вы знаете, когда я высадился на мысе Несбывшихся надежд, то на меня напали какие-то люди. Совершенно непонятно почему.";
		...
		break;
		case "DMHeadOver_3":
			dialog.text = "Отлично! Ну что же, всего вам доброго, сеньор.";
		...
		break;
		case "DMSkelHead_2":
			dialog.text = "Как-то причалил к нашему мысу пиратский бриг 'Королева'. Пираты здесь рабов контрабандистам продали. Договариваются между собой...\n"+
				"Продали пираты рабов, а потом у них между собой ссора какая-то вышла. Часть команды здесь осталась, а остальные с капитаном на бриге в море ушли. С той поры все и началось.";
		...
		case "DMSkelHead_6":
			...
			link.l1 = "Я не советую пытаться удержать меня силой. Прорвался сюда - вырвусь и отсюда...";
		...
		case "DMSkelHead_bye":
			dialog.text = "И вам того же, многоуважаемый капитан.";
		...
		case "DMSkelHead_8":
			...
			link.l1 = "Плохо... Может попытаться разыскать этот бриг, на котором ушли пираты? Похоже, что это единственная ниточка.";
		...
		case "LizBegin":
			...
			link.l1 = "Здравствуйте, Элизабет. Меня зовут " + GetFullName(pchar) + ". Я пришел переговорить с вами об одном... очень необычном деле.";
			link.l1.go = "LizBegin_1";
		break;
		case "LizBegin_1":
			dialog.text = "Слушаю вас внимательно, капитан.";
			link.l1 = "Некоторое время назад вы посетили небольшое испанское поселение - Дес-Мойнес. Глава этого небольшого города, Фернандо Торрес, с которым вы, полагаю, знакомы, попросил меня разыскать вас. Что я с удовольствием и сделал.";
		...
		case "LizBegin_3":
			...
			link.l1 = "Помогите мне разыскать бриг 'Королева'.";
			link.l1.go = "LizBegin_4";
		break;
		case "LizBegin_4":
			dialog.text = "Зачем ему нужен этот бриг?";
			link.l1 = "Дело в том, юная леди, что жители города связывают с этим бригом... возникновение внезапной эпидемии, поразившей город.";
		...
		break;
		case "LizBegin_6":
			dialog.text = "Конечно знаю! Дело в том, что капитан брига 'Королева' - очень дорогой мне человек. Последнее время я только тем и занимаюсь, что пытаюсь распутать этот клубок и развеять дьявольские чары.";
			link.l1 = "Получается?";
		...
		case "LizBegin_9":
			...
			link.l1 = "Я тоже это видел, Элизабет. Ходящие и разговаривающие человеческие скелеты - зрелище малоприятное. Так что же узнали мои коллеги у индейцев?";
		...
		case "LizBegin_15":
			...
			{
				link.l1 = "Ну что же, юная леди, я готов этим заняться, хотя бы в память о вашем отце. Значит, вы говорите, что этот череп находится в индейском городе?";
		...
		case "LizCheckSkull_1":
			dialog.text = "Теперь у нас есть инструмент для снятия проклятия. Вам нужно отыскать бриг 'Королева' и, собственно, снять проклятие с капитана.";
		...
		case "LizCheckSkull_2":
			...
			link.l1 = "Может быть... А где искать это бриг?";
			link.l1.go = "LizCheckSkull_3";
		break;
		case "LizCheckSkull_3":
			dialog.text = "Не знаю. Скорей всего, капитан блуждает по морю вдали от городов. Смотрите внимательней вокруг, этот бриг вы не пропустите.";
		...
		break;
		case "LizSeekQueen":
			dialog.text = "Капитан, вы нашли бриг 'Королева'?";
			link.l1 = "Нет еще...";
			link.l1.go = "exit";
			if (pchar.questTemp.MC == "over")
			{
		...
		break;
		case "DanAbordage_1":
			dialog.text = "Нам не о чем разговаривать. Ты напал на мой бриг!";
			link.l1 = "Мне нужно было попасть в капитанскую каюту, иного варианта у меня не было!.. Подождите, разве этот бриг ваш?";
			link.l1.go = "DanAbordage_2";
		break;
		case "DanAbordage_2":
			dialog.text = "Мой уже пять лет.";
			link.l1 = "Так вы и есть дорогой сердцу Элизабет человек... А я-то думал, что она говорит о мужчине!";
		...
		break;
		case "DanAbordage_4":
			dialog.text = "Точно! Позвольте вас спросить, кто вы такой?";
			link.l1 = "Меня зовут капитан " + GetFullName(pchar) + ". Я разыскал ваш бриг для того, чтобы снять проклятие. Расскажите мне, как получилось, что на вас пало проклятие ацтекского бога мертвых.";
		...
		break;
		case "DanAbordage_6":
			dialog.text = "Все стояли, как громом пораженные. Ни о каком поединке не могло быть и речи. Но и взять Кабаля на борт брига тоже было невозможно. Поэтому, с большей частью команды я ушла в море. С Пьером остались только те, кто ему симпатизировал.";
		...
		case "DanAbordage_10":
			...
			link.l1 = "Похоже, что дело уже сделано. Мне нужно было доставить нефритовый череп Миктлантекутли поближе к капитану брига 'Королева', что я сделал.";
		...
		case "DanAbordage_12":
			...
			link.l1 = "Благодарите сестру... Хотя, знаете, у меня есть к вам деловое предложение - идите ко мне офицером. Я пират, как и вы. Полагаю, мы сработаемся.";
...
void SetDanielleInWorld()
{
	...
	Log_TestInfo("Бриг Queen вышел из: " + sld.city + " и направился в: " + sld.quest.targetShore);
	//==> на карту
	sld.mapEnc.type = "trade";
	//выбор типа кораблика на карте
	sld.mapEnc.worldMapShip = "quest_ship";
	sld.mapEnc.Name = "Бриг 'Королева'";
