	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				{
					link.l4 = "Слушай, я не смог к полуночи быть в твоем погребе. Так что я никого не видел и нечего не услышал.";
					link.l4.go = "InterceptionLate";
				}
				if (pchar.questTemp.LSC == "InterceptionYouSeen")
				{
					link.l4 = "Ты знаешь, ровно в полночь я был в твоем погребе. Адмирал и еще какой-то тип собирались поговорить, но заметили меня и ушли. Так что я ничего не услышал.";
				...
				{
					link.l4 = "Знаешь, твой погреб - действительно хорошее место для подслушивания разговоров. Я дождался Чада.";
		...
		break;
		case "ansewer_8_2":
			dialog.text = "Нет, ты не обычный пират, ты - пиратский капитан. Вот, у тебя самого в команде мало было негодяев всех мастей?";
		...
		break;
		case "ansewer_9_1":
			dialog.text = "Ну, что тебе сказать? Все сундуки заперты на замки трех уровней сложности. Если простой ключ можно найти даже на улице, то с остальными куда сложней. Ведь именно в сундуки с хорошими замками люди прячу действительно ценные вещи.\n"+
		...
		break;
		case "ansewer_10":
			dialog.text = "Ха! Если бы я знал, приятель, кто такой Августо Брамс... А вообще я и знать этого не желаю.";
			link.l1 = "Объясни, пожалуйста.";
			link.l1.go = "ansewer_10_1";
			NextDiag.(NodePrevName).l10 = true;
		break;
		case "ansewer_10_1":
			dialog.text = "Хм, даже как он сюда попал - никто не знает, а ты хочешь, чтобы я тебе что-то объяснил. Он дает деньги в долг, но лучше от него держаться подальше, приятель.";
			link.l1 = "Ничего не понял, ну да ладно...";
			link.l1.go = "int_quests";
		break;
		case "ansewer_11":
		...
		break;
		case "ansewer_12_1":
			dialog.text = "Не стоит тебе копаться в этом деле, приятель, уж поверь мне.";
		...
		case "TizerCellar_2":
			...
			link.l1 = "К сожалению, мне лично с Тизером увидеться не довелось, иначе, без сомнения, я узнал бы все подробности этого дела.";
		...
		break;
		case "TizerCellar_5":
			dialog.text = "Одного твоего слова мало, приятель\nЯ вот что думаю. Ты должен отправиться на флейт 'Фернандо Диффиндур'. Место это опасное чрезвычайно - корабль треснул пополам и непонятно как держится на поверхности. Однако именно там Тизер Дэн проводил очень много времени. Полагаю, что именно там мы сможем найти ключ к тайне Тизера.";
		...
		case "TizerCellarOk_3":
			dialog.text = "Хе-хе, без сомнения! Я неплохо знал Тизера, он ни за что на свете не сумел бы построить и дырявого плота - руки у него не из того места росли. Но раз он сумел-таки добраться до Большой земли, значит, его помощник, как пить дать, является специалистом корабельном деле\nХа! Да ведь это наш плотник, Андре Лабор!! Вот что, дружище, отправляйся к нему. Поговори с ним тихо и мирно, расскажи о Тизере Дэне. А то ведь он и не знает, что этот бродяга все-таки добрался до Большой земли!";
		...
		case "AfterProtector_1":
			...
			link.l1 = "Хм... Ну, в общем, поговорил я с плотником, этим Андре Лабором, и он попросил меня пройти в каюту корвета 'Протектор'. Я туда зашел, ну, и жду его, ничего такого не думаю даже... А этот Андрэ привел милиционера на разговор со мной! Пришлось всех отправить на тот свет.";
		...
		case "AfterProtector_3":
			...
			link.l1 = "Милиционер сам так сказали.";
		...
		case "AfterProtector_7":
			dialog.text = "У всех членов клана 'Нарвал' есть амулет с изображением перевернутой пятиконечной звезды. Достань амулет и иди с ним к адмиралу. Расскажешь ему о том, что нашел его на месте убийства милиционера и плотника.";
		...
		case "AfterNarvalDesrt_2":
			dialog.text = "Н-да, лихо... Ну, братец, медаль тебе положена. Хотя, если как следует поразмыслить, вреда от твоего деяния больше, чем пользы.";
			link.l1 = "Как понять?";
		...
		case "AfterNarvalDesrt_5":
			dialog.text = "Что теперь думаешь делать? Ты ведь теперь герой.";
			link.l1 = "Не знаю. Похоже, что я добился уважения у Чада. Вообще, мне здесь начинает нравиться. Может, в милицию запишусь...";
			link.l1.go = "AfterNarvalDesrt_6";
		break;
		case "AfterNarvalDesrt_6":
			dialog.text = "Вот что, приятель, ты пока погуляй, но не забывай наведываться ко мне. Я тут постараюсь выяснить кое-что...";
			link.l1 = "Что именно?";
		...
		case "InterceptionOk":
			...
			if (sld.location == "FleuronTavern")
				dialog.text = "Говори тише, приятель! У меня тут адмирал как раз прохлаждается... Ну что, сумел разобрать разговор?";
			else
				dialog.text = "Сумел разобрать разговор?";
			link.l1 = "Сумел. Оказывается, адмирал сам собирается покинут Город!";
		...
		case "InterceptionOk_6":
			dialog.text = "А с чего ты взял, что этот твой Механик существует? Может, ты неверно понял смысл сказанного адмиралом?";
		...
		break;
		case "NB_4":
			dialog.text = "Да. Как только все стало известно, адмирал вызвал меня и попросил заняться делом Брюннера. Без таверны, сам понимаешь, жизни нет...";
		...
		case "NBB_1":
			dialog.text = "Мертвее не бывает, дружище.";
		...
		case "ADied_3":
			...
			link.l1 = "Из-за меня все. Не сумел я предусмотреть такого. Дьявол, никогда себе не прощу!";
			link.l1.go = "exit";
		break;
		//поиск товаров на корвет
		case "SeekGoods":
		...
		case "SeekCrew_1":
			...
			link.l1 = "Есть. Иначе я бы тебя не звал.";
