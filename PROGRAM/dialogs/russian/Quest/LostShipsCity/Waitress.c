...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				dialog.Text = "Ах, месье капитан, вы уже знаете о нашем несчастье?";
			...
			{
				dialog.Text = "Ах, месье капитан, какой кошмар! Какой ужас!!";
			...
			else
			{
				...
				{
					link.l4 = "Армо, прошу прощения, но я, кажется, все-таки опоздал к свиданию.";
		...
		case "int_quests":
			...
			{
				link.l7 = "Я слышал, ты встречаешься в Томасом Бойлом.";
		...
		break;
		case "ansewer_5_1":
			dialog.text = "На галеоне 'Эсмеральда', месье капитан.";
			link.l1 = "Понял. Огромное тебе спасибо, " + npchar.name + ".";
			link.l1.go = "int_quests";
		break;
		case "ansewer_6":
			dialog.text = "Месье капитан, я ничего не знаю о Лейтоне Декстере. И знать не желаю.";
			link.l1 = "Хм, ну, извини...";
			link.l1.go = "int_quests";
			NextDiag.(NodePrevName).l6 = true;
		break;
		case "ansewer_7":
			dialog.text = "Да, это так. А зачем вы спрашиваете, месье капитан?";
			link.l1 = "Да так, просто... Рад за вас!";
			link.l1.go = "int_quests";
		...
		break;
		case "NB_12":
			dialog.text = "Хм... Вы знаете, месье капитан, мне сейчас не до шуток.";
			link.l1 = "А я и не шучу... Ладно, Армо, не печалься. Жизнь такая штука - люди умирают...";
			link.l1.go = "NB_13";
		break;
		case "NB_13":
			dialog.text = "Хорошо, если своей смертью!.. Месье капитан, обещайте мне, что если вы что-нибудь узнаете об этом деле, то сообщите мне.";
			link.l1 = "Обещаю, Армо.";
			link.l1.go = "NB_14";
		break;
		case "NB_14":
			dialog.text = "Спасибо. Я бы очень хотела знать, кто это сделал. Это негодяй не имеет права жить!";
			link.l1 = "Да-да, я с тобой согласен...";
			link.l1.go = "exit";
		...
		case "AboutCasper_1":
			dialog.text = "Говорите, месье капитан.";
			link.l1 = "Похоже, что за этим стоит клан 'Каспер'.";
			link.l1.go = "AboutCasper_2";
		break;
		case "AboutCasper_2":
			dialog.text = "Вы уверены?";
			link.l1 = "Да, уверен. Я пошел к адмиралу, задавал ему вопросы относительно расследования убийства Хилла, хотел помочь. В итоге, мне было сказано, чтобы я не лез в это дело. Прямо и откровенно.";
		...
		break;
		case "AboutCasper_5":
			dialog.text = "Вы собираетесь делать это один?";
			link.l1 = "Ну да. Адмирал на этот раз помогать мне не будет.";
			link.l1.go = "AboutCasper_6";
		break;
		case "AboutCasper_6":
			dialog.text = "Они вас убьют. В этом нет никаких сомнений, месье капитан.";
			link.l1 = "Ну, это мы еще посмотрим...";
			link.l1.go = "AboutCasper_7";
		break;
		case "AboutCasper_7":
			dialog.text = "Вот что, я знаю, чем смогу вам помочь. Дело в том, что я ношу на 'Сан Габриль' еду для этих мерзавцев. Моих поклонников там - хоть отбавляй. Я буду назначать им свидания, а вы будете разбираться с ними один на один.";
			link.l1 = "Хо! Неплохо придумано...";
			link.l1.go = "AboutCasper_8";
		break;
		case "AboutCasper_8":
			dialog.text = "Где вам будет удобно проводить эти встречи?";
			link.l1 = "Хм, не знаю. Мне, собственно, все равно, где встречаться с мужчиной...";
		...
		case "Casper1":
			...
			{
				dialog.text = "Сегодня, в полночь 'каспер' по имени Денис Хопфер будет добиваться моего расположения на квартердеке флейта 'Церес Смити'. Будте острожны, месье капитан, если вы нападете на Хопфера слишком близко к 'Сан Габрилю', то звуки драки могут услышать остальные бандиты, и тогда вам конец.";
				link.l1 = "Я понял, Армо.";
		...
		case "Casper1_1":
			...
			link.l1 = "Хе-хе, это верно. В полночь буду на месте как штык!";
			link.l1.go = "exit";
		...
		case "Casper2":
			if (GetQuestPastDayParam("questTemp.LSC") > 0 && GetHour() < 20)
			{
				...
				link.l1 = "Хе, недолго ему осталось досажадать тебе.";
				link.l1.go = "Casper2_1";
			}
			else
			{
				dialog.text = "Пока нет, месье капитан. Ожидайте.";
				link.l1 = "Ясно.";
				link.l1.go = "exit";
			}
		break;
		case "Casper2_1":
			dialog.text = "Хорошо, это радует. Итак, месье капитан, не опазывайте на квартердек флейта 'Церес Смити'. Думаю, что не стоит разочаровывать Сержа.";
		...
		case "WaitCasper2":
			dialog.text = "Очень хорошо, месье капитан. Еще попробуем провернуть это дело?";
		...
		break;
		case "Casper3_1":
			dialog.text = "Прекрасно! Что ж, месье капитан, удачи вам.";
		...
		case "CasperLate":
			dialog.text = "Знаете, что, месье капитан, я не для того рискую своей жизнью, чтобы так легкомысленно относились к своим обещаниям. Я не намерена более помогать вам. Извините.";
			link.l1 = "Эх, жаль...";
			link.l1.go = "exit";
		...
		break;
		case "CasperAllFight":
			dialog.text = "Да, я знаю. На вас напал весь клан разом, наверное, услышали шум вашей схватки. Главное, что вы справились. Я рада за вас, месье капитан.";
		...
		//Армо мертва
		case "ArmoIsDead":
			dialog.text = "Здравствуйте. Я вас знаю, тот самый новые гражданин, с которым Армо была очень дружна.";
			link.l1 = "Да, это так.";
		...
		break;
		case "ArmoIsDead_2":
			dialog.text = "Ну, слезами горю не поможешь, как говорится... Знаете, в память об Армо, я буду назвать вас так же, как она - месье капитан. Вы не против?";
			link.l1 = "Конечно, нет.";
			link.l1.go = "exit";
			NextDiag.TempNode = "First time";
		break;
		...
		case "askMoney":
			dialog.text = "Вам нужны деньги, месье капитан? А зачем?";
			link.l1 = "На одно очень нужное для всего Города дело. Вопрос жизни и смерти!";
			link.l1.go = "askMoney_1";
		break;
		case "askMoney_1":
			dialog.text = "Я верю вам, месье капитан. Я могу вам отдать мои сбережения, тем более, что мне они ни к чему.";
		...
		break;
		case "SeekGoods_1":
			dialog.text = "Бомбы и прочее можно спросить у хозяек в кораблях. Если ты с ними не знаком, то это Элис Тейлор, Орели Бертин, Леа Тоорс и Элиза Кальво. Об остальном понятия не имею.";
		...
		case "SeekCrew_1":
			if (npchar.name == "Армо")
			{
				dialog.text = "Нет, месье капитан. Мы с Томасом остаемся в Городе.";
				link.l1 = "Да при чем тут твой Томас?! Оставаясь здесь, ты рискуешь жизнью.";
				link.l1.go = "SeekCrew_2";
			}
			else
			{
				dialog.text = "Нет, месье капитан. Я останусь здесь, в своей таверне.";
		...
		//освобождение мужа Элис Тейлор
		case "ELTHusb_SF":
			dialog.text = "Помню, месье капитан.";
		...
		break;
		case "ELTHusb_SF_2":
			dialog.text = "С Жилем Бару по под ручку.";
