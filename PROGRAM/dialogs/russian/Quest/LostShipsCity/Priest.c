...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "FT_1":
			dialog.text = "Я вижу, сын мой, что душа твоя неспокойна. Ибо нельзя за несчастия свои вину возлагать на Господа...";
		...
		break;
		case "FT_2":
			dialog.text = "Сын мой, разве ты безгрешен?";
		...
		case "LSCNotBlade":
			dialog.text = LinkRandPhrase("Убери оружие, сын мой. А то нервируешь немного...", "Знаешь, у нас тут не принято сабелькой размахивать. Убери оружие.", "Убери оружие, не к лицу это серьезному мужчине...");
		...
		//Исповедь
		case "donation":
			dialog.Text = "Конечно, сын мой. Как много ты хочешь пожертвовать во искупление грехов твоих?";
			Link.l1 = "Простите, святой отец, я передумал. На мели...";
		...
		case "donation paid_100":
			...
			dialog.Text = "От лица прихожан Города благодарю тебя, сын мой.";
		...
		case "donation paid_5000":
			...
			dialog.Text = "От лица прихожан Города благодарю тебя, сын мой, за щедрый дар.";
			Link.l1 = "Да не за что...";
			Link.l1.go = "exit";
			AddDialogExitQuest("donation");
		break;
		...
		//достать свечки
		case "askCandles":
			dialog.text = "Да, сын мой. Приход нуждается в свечах, а у нас в Городе что-то давно не был поступлений этого товара. Помоги мне, принеси 400 свечей.";
			link.l1 = "И где мне их взять?";
			link.l1.go = "askCandles_1";
		break;
		case "askCandles_1":
			dialog.text = "Если бы я знал, где взять, то сам бы и сходил за ними. Поищи, сын мой.";
			link.l1 = "Хм... Ну, хорошо, святой отец, буду искать...";
			link.l1.go = "askCandles_2";
		break;
		case "askCandles_2":
			dialog.text = "Ищи прилежно сын мой, награда будет достойной.";
			link.l1 = "Я понял, падре. Все будет сделано.";
			link.l1.go = "exit";
		...
		case "takeCandles":
			...
			{
				dialog.text = "Ну, давай посмотрим. Сейчас ты принес " + FindRussianQtyString(iTemp) + " свечей. Всего ты принес " + FindRussianQtyString(sti(npchar.quest.takeCandles.Qty)) + " свечей. Тебе осталось принести еще " + FindRussianQtyString(400 - sti(npchar.quest.takeCandles.Qty)) + ".";
			...
			{
				dialog.text = "Отлично, ты выполнил поставленную задачу в полном объеме, сын мой. Теперь нам хватит свечей для церковных служб на долгое время.";
				link.l1 = "Я рад, что смог помочь приходу. Святой отец, а что там насчет награды за труды?";
				link.l1.go = "takeCandles_ok";
			}
		break;
		case "takeCandles_ok":
			dialog.text = "Да, конечно, как и обещал. Я думаю, ты сам почувствуешь изменения в твоей жизни здесь. Иди, сын мой.";
		...
		case "SeekGoods":
			dialog.text = NPCStringReactionRepeat("О чем ты, сын мой?", 
				"Ты уже спрашивал об этом.", 
				"Уже спрашивал...",
                "Сын мой, хватит уже...", "block", 0, npchar, Dialog.CurrentNode);
		...
		break;
		case "SeekGoods_2":
			dialog.text = "Хм, лекарства у меня имеются. Только для начала окажи и мне услугу сын мой. Я говорю о свечах.";
			link.l1 = "О свечах?";
		...
		case "FSeekGoods":
			dialog.text = NPCStringReactionRepeat("Как много?", 
				...
				"Уже спрашивал...",
                "Сын мой, хватит уже...", "block", 0, npchar, Dialog.CurrentNode);
			link.l1 = HeroStringReactionRepeat("50 центнеров, полагаю, будет достаточно.", 
				"Да... Извините, запамятовал.",
                "А-а, ну да.", 
				"Понял, понял...", npchar, Dialog.CurrentNode);
		...
		break;
		case "FSeekGoods_1":
			dialog.text = "А зачем тебе, сын мой?";
			link.l1 = "Увы, святой отец, сказать пока не могу.";
			link.l1.go = "FSeekGoods_2";
		break;
		case "FSeekGoods_2":
			dialog.text = "Ну что же, я не любопытен, ибо это грех... Ты можешь забрать у меня необходимую тебе партию лекарств. Как-то ты помог мне, теперь я помогу тебе.";
		...
		break;
		case "FSeekGoods_3":
			dialog.text = "Хорошо. Иди с миром, сын мой, прихожане все сделают.";
		...
		//найм команды
		case "SeekCrew":
			dialog.text = NPCStringReactionRepeat("В чем дело, сын мой?", 
				"Мы уже говорили об этом.", 
				"Это уже обсуждено, второй раз повторяю, сын мой.",
                "Ох, сын мой, какой же ты надоедливый...", "block", 0, npchar, Dialog.CurrentNode);
		...
		break;
		case "SeekCrew_2":
			dialog.text = "На все воля Господа, сын мой. Я остаюсь.";
