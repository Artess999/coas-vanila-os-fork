	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Ага, так это ты из шторма вышел? Ну что же, отлично! Давай знакомиться. Меня зовут " + GetFullName(npchar) + ", а тебя?";
			...
			else
			{
				...
				{
					dialog.text = "Приятель, не желаешь провернуть одно дельце?";
				...
				{
					link.l4 = "Ну что, я готов зайти в эту устрашающую дверь.";
		...
		break;
		case "ansewer_5":
			dialog.text = "Случаются, чего уж... Кстати, тебе надо поговорить с адмиралом Чадом Каппером, если ты до сих пор этого не сделал.";
			link.l1 = "Понятно.";
		...
		break;
		case "ansewer_2":
			dialog.text = "Во-первых, тебе нужно побыстрей явиться к адмиралу Города, если еще не был у него. Ну а потом можно наведаться в таверну и магазин. Еще есть церковь.";
			link.l1 = "Понятно...";
			link.l1.go = "int_quests";
		...
		case "StBegin_1":
			dialog.text = "Ест тут одно место, где много денег имеется...";
			link.l1 = "И что?";
		...
		break;
		case "StBegin_not":
			dialog.text = "Ну, как знаешь, приятель...";
			link.l1 = "Ага, бывай.";
			link.l1.go = "exit";
			npchar.quest.store = "over";
		break;
		case "StBegin_6":
			dialog.text = "Отлично! Итак, договоримся таким образом. Ты сейчас иди по своим делам, как будешь готов - дуй к 'Эсмеральде', днем я буду тебя там ждать на квартердеке рядом с дверью, над которой черепушка прибита. Там и разберемся, что к чему...";
		...
		break;
		case "StResult":
			dialog.text = "Ну что, приятель, дело сделано, как я погляжу?";
			link.l1 = "Да, все в порядке, 'причесал' там двоих охранников.";
			link.l1.go = "StResult_1";
		break;
		case "StResult_1":
			dialog.text = "Ну, иного я и не ожидал. То, что ты хороший воин - видно с полувзгляда... А теперь давай делить добычу, дружище.";
			if (sti(pchar.money) <= sti(npchar.quest.store.money))
			{
				link.l1 = "Слушай, ничего я там не нашел. Не было там денег. Все обыскал, а денег не нашел.";
				link.l1.go = "StResult_2";
			}
			else
			{
				npchar.quest.store.money = sti(pchar.money) - sti(npchar.quest.store.money); //чистая выручка
				link.l1 = "Всего я нашел там " + FindRussianMoneyString(sti(npchar.quest.store.money)) + ". Половина, как и договаривались, твоя.";
		...
		break;
		case "StResult_3":
			dialog.text = "Ну, вот и отлично! Рад, что все так удачно завершилось... Ну, бывай, приятель. Будут проблемы - обращайся.";
		...
		break;
		case "SeekCrew_1":
			dialog.text = "Дружище, у тебя что, действительно есть корабль, на котором можно уплыть отсюда?";
			link.l1 = "Да.";
