...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":			
			...
			{
				dialog.text = "Здравствуй. Ты что-то хотел?";
				link.l1 = "Мне нужны деньги.";
				link.l1.go = "GiveMoney";
				link.l10 = "Нет, просто так заскочил к тебе.";
				link.l10.go = "Exit";
		...
		break;
		case "Man_FackYou":
			dialog.text = "Ты, приятель, не знаешь, с кем связался! А теперь тебе конец...";
		...
		break;
		case "SeekCrew_1":
			dialog.text = "Мне многое известно... Ты хотел позвать меня с собой?";
		...
		case "GiveMoney":
			if (CheckAttribute(npchar, "quest.loan"))
			{
				...
				{
					dialog.text = "Ты полностю исчерпал выделенную квоту. Кредитная линия тебе закрыта.";
					link.l1 = "Ясно.";
					link.l1.go = "exit";
				}
				else
		...
		break;
		case "GiveMoneyKn_2":
			dialog.text = "У меня нет процентовки, завязанной на времени. Ты просто должен будешь вернуть мне в пять раз большую сумму по первому требованию моего доверенного лица.";
			link.l1 = "Чего?!!!";
			link.l1.go = "GiveMoneyKn_3";
		break;
		case "GiveMoneyKn_3":
			dialog.text = "У тебя проблемы со слухом?";
			link.l1 = "Нет, просто я немного опешил...";
		...
		case "GiveMoney_2":
			...
			else
			{
				if (iTemp <= 0)
				{
					dialog.text = "Шутник, да? Не советую дурачиться со мной.";
					link.l1 = "Хм, понял...";
					link.l1.go = "exit";
				}
				else
		...
		break;
		case "GiveMoney_3":
			dialog.text = "Очень рад. Надеюсь, что ты будешь помнить об условиях рассчета за предоставленный займ. Когда придется время, ты должен будешь вернуть сумму, в пять раз превышающую размер долга. И на данный момент, это " + FindRussianMoneyString(sti(npchar.quest.loan.qtyMoney) * 5) + ".";
			link.l1 = "Да, я помню. А что будет, если я не смогу их сразу вернуть?";
			link.l1.go = "GiveMoney_4";
		break;
		case "GiveMoney_4":
			dialog.text = "Хм, лучше, если бы они у тебя были, приятель. Альтернатива, мягко говоря, очень страшна. Очень...";
