	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			if (npchar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Чертов шторм! Хотя уже пошел на убыль... Я так понимаю, что ты - новый гражданин Города.";
					link.l1 = "Капитан " + GetFullName(pchar) + " собственной персоной. А шторм - не беда!";
					link.l1.go = "FS_1";
				}
				else
				{
					dialog.text = "А-а, ну вот ты и ко мне пожаловал. Я так понимаю, что ты - новый гражданин Города.";
			...
			else
			{
				...
				{
					link.l2 = "Дружище, я слышал, что у тебя есть карта Города. Ты знаешь, в данный момент мне бы она очень пригодилась. А то бегаю здесь как слепой котенок, не освоился еще...";
		...
		break;
		case "Man_FackYou":
			dialog.text = LinkRandPhrase("Какого черта ты лезешь ко мне в сундук! Закона Города не знаешь? Ну ладно, теперь узнаешь...", "Эй, гражданин, ты что это вытворяешь? Закон запрещает тебе так поступать! Я тебя накажу...", "Постой, ты куда это полез? У нас в Городе завелся вор!! Ну, теперь держись...");
		...
		//поиск товаров на корвет
		case "SeekGoods":
			dialog.text = "Всегда к твоим услугам, приятель. Заказывай!";
			link.l1 = "Слушай, таких товаров в твоем ассортименте я еще не встречал. Это бомбы, книпеля, порох, провиант, парусина, доски, лекарства...";
			link.l1.go = "SeekGoods_1";
		break;
		case "SeekGoods_1":
			dialog.text = "Ну, из того, что ты перчислил, здесь в цене только провинат и парусина, доски и лекарства. Остальное просто никому не нужно.";
			link.l1 = "А у тебя что-нибудь есть?";
			link.l1.go = "SeekGoods_2";
		break;
		case "SeekGoods_2":
			dialog.text = "Провиант тебе могу продать. Он довольно дорогой.";
			link.l1 = "Мне нужно 200 центнеров Сколько это будет стоить?";
		...
		case "SeekGoods_5":
			dialog.text = "Отлично, дружище. Куда тебе все это сгрузить?";
			link.l1 = "Слушай отправь все это на 'Сан Габриэль'. 'Касперов' там уже нет, если ты в не курсе...";
		...
		case "SeekGoodsOk":
			dialog.text = "Раздобыл деньги?";
			if (sti(pchar.money) >= 40000)
			{
				link.l1 = "Да. И готов заплатить.";
				link.l1.go = "SeekGoods_5";
			}
			else
			{
