...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "ClimeUsurer":
			dialog.text = "Ты задолжал моему адепту по имени Августо Брамс!";
			link.l1 = "Я?.. А-а, ну да, было дело... Гм, но только я не знал, что он твой адепт.";
			link.l1.go = "ClimeUsurer_1";
		break;
		case "ClimeUsurer_1":
			dialog.text = "Тем не менее, это так. Ты должен отдать мне немедленно " + FindRussianMoneyString(sti(pchar.questTemp.LSC_climeUsurer.qtyMoney )) + ". У тебя есть такие деньги?";
			...
			{
				link.l1 = "Как-то все это неожиданно... Я же не знал, что так быстро надо будет рассчитаться.";
		...
		case "InGreateTemple":
			dialog.text = "Как ты посмел, смертный?!!";
			link.l1 = "Оба-на!.. Если не ошибаюсь, то вы и есть тот самый бог мертвых.";
			link.l1.go = "InGreateTemple_1";
		break;
		case "InGreateTemple_1":
			dialog.text = "Это так\nЯ давно наблюдаю за тобой, твое рысканье по Теночтитлану занимало меня. Но сейчас ты взял предмет моего культа, и это уже не смешно...";
		...
		case "InGreateTemple_2":
			dialog.text = "Монтесума? Ха-ха, никак не угомониться. Хотя, иначе и быть не может.\nНу что же, ты дотронулся до нефритового черепа, а это является преступлением. Но ты сумел нейтрализовать охрану, которая состоит их героев прошлого\nБолее всего на свете ацтеки почитают воинскую доблесть. Ты показал, что являешься хорошим воином, поэтому я дам тебе шанс выжить. Докажи, что ты великий воин, и ты сможешь уйти отсюда.";
		...
		case "WinInTemple":
			dialog.text = "Ну что же, ты доказал, что достоин. Теперь ты можешь уйти отсюда живым.";
			link.l1 = "Уф-ф-ф... Если хотите, я положу череп на место.";
			link.l1.go = "WinInTemple_1";
		break;
		case "WinInTemple_1":
			dialog.text = "Не надо, у меня таких много. Череп можешь взять себе, ты это заслужил.";
