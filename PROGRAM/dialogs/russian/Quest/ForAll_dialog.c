...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Begin_PoorKill_4":
			dialog.text = "Да, собственно, пустяк для такого храброго человека, как вы. Дело в том, что мой работодатель всерьез озаботился состоянием морали и нравственности на островах Карибского моря. Да-да, именно так!";
		...
		case "Begin_PoorKill_7":
			dialog.text = "Дорогой мой, он не валяется и вне дорог. Миллионы вообще не имеют свойства валяться ни вместе, ни по отдельности, хе-хе...";
		...
		case "BlueBirdCapitain_1":
			dialog.text = "И какого дьявола ты напал на мою шебеку?!!";
		...
		case "DeckSharp":	//на палубе
			dialog.text = "Здравствуй, приятель. Чему обязан за удовольствие видеть тебя на моем бриге?";
			link.l1 = "Приветствую капитана Бартоломью Шарпа! С трудом разыскал тебя...";
		...
		case "NewDeckSharp_noMoney":
			if ((sti(pchar.questTemp.Sharp.price)*10) < sti(pchar.money))
			{
				...
				{
					dialog.text = "Че-то я не понял, ты меня за лоха держишь? Ты чего меня лечишь, думаешь, я не знаю, что денег у тебя прилично имеется? Знаю, приятель, это мне прекрасно известно. Так что, давай, раскошеливайся или ищи ловцов жемчуга где-нибудь на зюйд-зюйд-ост отсюда, к примеру...";
		...
		case "CitySharp":
			if (npchar.quest.meeting == "0")
			{
				...
				link.l1 = "Бартоломью, я так рад встрече! Пришлось тебя поискать, но в этом городе повезло...";
			...
			else
			{
				...
				link.l1 = "Да, Бартоломью, это я собственной персоной...";
		...
		break;
		case "PortmansCap_2":
			dialog.text = "Дьявол, так вот где я его 'посеял'! Да уж, это пропажа чуть было не привела к бунту на корабле...";
		...
		case "PortmansCap_inDeck_5":
			dialog.text = "Абсолютно, дружище. Ты слишком долго меня искал, нужно было поторопиться.";
		...
		case "SeekCap_inDeck":
			...
			link.l1 = "Да так, новостями обменяться хотел. Может, что продается у тебя...";
		...
		case "SCQ_RapeWife_1":
			...
			dialog.text = "Я небогат, но с радостью отдам все, что у меня есть! Если вы освободите мою жену и доставите ее ко мне, то я заплачу вам " + FindRussianMoneyString(sti(npchar.quest.money)) + ".";
		...
		break;
		case "SCQ_RapeWife_4":
			dialog.text = "Спасибо вам огромное! Я буду ждать вам с местной церкви. Только я умоляю вас - быстрее. Я очень опасаюсь за жену...";
		...
		case "SCQR_manRapeWife":
			dialog.text = "Боже мой, вы буквально вернули меня к жизни! Извольте получить свои " + FindRussianMoneyString(sti(npchar.quest.money)) + ". И знайте, что мы будем молиться за вас до конца жизни!";
		...
		case "SCQ_Hasband_2":
			...
			link.l1 = "Понятно. Ну что же, теперь ожидайте. И постарайтесь находиться преимущественно в церки, чтобы я не искал вас по городу...";
			link.l1.go = "exit";
			//==> ставим квестодателя в церковь
			pchar.quest.SeekShip_Stay.win_condition.l1 = "ExitFromLocation";
			npchar.location = npchar.city + "_church";
		...
		case "SCQ_Pirates_5":
			...
			link.l2 = "Сожалею, но это слишком мало для того, чтобы я напал на своего коллегу, члена Берегового братства.";
			link.l2.go = "SCQ_exit_clear";
		break;
		case "SCQ_Pirates_6":
		...
		case "SCQ_Pirates_7":
			...
			link.l1 = "Ясно. Ну что же, теперь ожидайте и надейтесь, что мне повезет с поисками. Постарайтесь находиться в церкви, чтобы я мог вас там быстро найти.";
			link.l1.go = "exit";
			//==> ставим квестодателя в церковь
			pchar.quest.SeekShip_Stay.win_condition.l1 = "ExitFromLocation";
			npchar.location = npchar.city + "_church";
		...
		break;
		case "CCmanSlave_3":
			dialog.text = "Ты подумай, дружище, ради кого ты стараешься! Это же совершенно ничтожный человечишка. Ему самое место - на плантациях тростника!";
		...
		break;
		case "RevengeCap_5":
			dialog.text = "Да какой ты мне друг, идиот?! Ты и капитаном-то назваться не можешь. Так, прихвостень на побегушках у вздорной фурии!";
			link.l1 = "Вот как?!";
		...
		case "RevengeCap_inDeck_2":
			...
			link.l1 = "Ну что же, прощайте. Но имейте в виду то, что я вам сказал.";
		...
		//несчастный мушкетер
		case "OffMushketer":
			dialog.text = "Приветствую гостя в моем доме. Что вы хотели здесь?";
		...
		case "OffM_10":
			...
			link.l1 = "Ну что же, я вас понял. А кто захватил ваш корабль, когда это было?";
			link.l1.go = "OffM_11";
		break;
		case "OffM_11":
		...
		case "OffM_result":
			...
			{
				link.l1 = "Да, дружище, нашел. Вот он, двуствольный мушкет, другого такого нет.";
...
void SetMushketCapitainInWorld()
{
	...
	sld.mapEnc.Name = "Бригантина 'Стрела'";
	int daysQty = GetMaxDaysFromIsland2Island(GetArealByCityName(sld.quest.targetCity), GetArealByCityName(sld.city))+5; //дней доехать даем с запасом
