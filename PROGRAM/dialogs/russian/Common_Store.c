	...
	switch(Dialog.CurrentNode)
	{
		...
		case "second time":
			...
			link.l1 = NPCharRepPhrase(npchar,
						...
						pcharrepphrase(RandPhraseSimple("К черту такие разговоры, ","Поворачивай на другой галс, ") +GetFullName(Npchar)+ "! "
						              ...
						              +RandPhraseSimple(" Я пришел в магазин!"," А зачем ты здесь сидишь?"),
		...
		case "node_1":
			dialog.text = NPCStringReactionRepeat(NPCharRepPhrase(npchar,
			                                         pcharrepphrase(RandPhraseSimple("Свистать всех наверх! ","Трюм к осмотру! ") +LinkRandPhrase("Что нужно джентльмену удачи в моем магазине, хе-хе?",
