...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "hired":
			...
			// только наняли <--
			dialog.text = NPCharSexPhrase(NPChar, "Что вы хотите, капитан?", "Я готова поговорить с вами, капитан.");
			if (isOfficer(Npchar)) // проверка для кампуса, где слотовые офы
			{
	            Link.l2 = "Слушай мой приказ!";
	            Link.l2.go = "stay_follow";
            }
			Link.l4 = "Ты уволен" + NPCharSexPhrase(NPChar, "", "а") + ". Я обойдусь без твоих услуг!";
        ...
        case "WantToGo":
			...
			dialog.text = LinkRandPhrase("Капитан, все люди как люди: влюбляются, женятся, дома сидят, одни мы, словно заведенные, по морю шныряем. Хочу другой жизни вкусить немного. Отпусти, будь человеком!",
                           "Капитан, по морям шататься с вами, конечно, неплохо, но надо и о себе когда-то подумать. Домишко купить, деток завести. А на это нужно время. Вы уж отпустите меня. Хочу пожить, как нормальный человек.",
		...
		case "Fired":
		      ...
		      Link.l1 = "Да ты мне и не " + NPCharSexPhrase(NPChar, "нужен", "нужна") + "!";
		      Link.l1.go = "Exit";
	      break;
        // boal 05.09.03 offecer need to go to abordage -->
	      case "Boal_Stay":
