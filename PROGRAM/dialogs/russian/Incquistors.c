		...
		case "First time":
			...
			else
			{
				if (pchar.questTemp.MC == "toByeBye" && findsubstr(npchar.id, "Incquisitor_", 0) != -1)
				{
					...
					link.l1 = "Святой отец, у меня есть к вам дело. Не откажите ревностному католику в минуте внимания.";
					link.l1.go = "MCIncq";
...
//**************************** сдаем зачарованный Дес-Мойнес ********************************
		case "MCIncq":
			dialog.text = "Слушаю тебя, сын мой.";
		...
		break;
		case "MCIncq_2":
			dialog.text = "Ага... Мы займемся этим, сын мой. Господь благодарит тебя за верную службу церкви.";
			link.l1 = "Рад стараться!";
			link.l1.go = "MCIncq_3";
		break;
		case "MCIncq_3":
			dialog.text = "А чтобы ты и в дальнейшем старался как следует, прими в награду за труды 10 тысяч пиастров.";
			link.l1 = "Премного благодарен, ваше святейшество!";
			link.l1.go = "exit";
