...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "ToPrisonHead_expansive":
			...
			link.l1 = "Залог за твое освобождение слишком велик, у меня нет таких денег. Я вынужден отказаться от помощи тебе.";
			link.l1.go = "ToPrisonHead_notFree_2";
			pchar.quest.GivePrisonFree_Over.over = "yes";
		break;
        ...
        case "PrisonerInPlace_3":
			dialog.text = "Конечно, приятель! Итак, я закопал здесь всякой всячины на сумму " + FindRussianMoneyString(sti(pchar.questTemp.jailCanMove.Sum)*2) + ". Твоя доля составляет половину, как и было уговорено. Изволите получить наличными, " + GetAddress_Form(NPChar) + "?";
			link.l1 = "Естественно!";
