...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "ResMan":
			...
			if (isBadReputation(pchar, 40))
			{
				dialog.text = LinkRandPhrase("Пошел вон отсюда, мерзавец!", "Немедленно убирайся из дома губернатора, негодяй!", "Мне нет дела до того, что тебе нужно в доме губернатора " + XI_ConvertString("Colony" + npchar.city + "Gen") + ". Но дам тебе совет - убирайся отсюда!");
		...
		break;
		case "Man_FackYou":
			dialog.text = "Да ты никак вздумал обворовать губернатора?! Охрана, держи вора!!";
