...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "convoy_DeskTalk":
            ...
            {
				link.l1 = HeroStringReactionRepeat(RandPhraseSimple("Прошу меня простить, дела... замотался... Через 7 дней вы точно будете на месте.", "Извините " + GetAddress_Form(NPChar) + ", совсем не было возможности выполнить обязательства по отношению к вам. Но можете не переживать, через неделю будем на месте."),
					RandPhraseSimple("Я вынужден во второй раз извиниться... Через неделю мы будем на месте.", "Я вторично приношу извинения. Через неделю мы будем на месте."),
					RandPhraseSimple("И в третий раз я вынужден просить прощения у вас... Мы будем на месте через неделю.", "Я приношу извинения в третий раз. Через неделю мы будем на месте, обещаю..."),
			...
			link.l2 = HeroStringReactionRepeat(RandPhraseSimple("А-а... пассажир... я и забыл про вас. Думаю, в трюме вам будет спокойнее, и доставать меня вы прекратите.", "Хех, ну надо же! У меня на корабле - посторонние!.. Полагаю, тебе нужно познакомиться с моим трюмом."),
		...
		case "convoy_Prison_3":
			AddSimpleRumour(LinkRandPhrase("Стало известно, что некий капитан " + GetFullName(pchar) + " взялся доставить пассажира по имени " + GetFullName(npchar) + ", но слова своего не сдержал. К тому же пленил беднягу. Вот так - доверять малознакомым людям...", 
