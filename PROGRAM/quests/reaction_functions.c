...
void SetDanielleFromSeaToMap(string qName)
{
	...
	sld.quest.targetCity = SelectAnyColony(sld.city); //определим колонию, в бухту которой он придет
	sld.quest.targetShore = GetIslandRandomShoreId(GetArealByCityName(sld.quest.targetCity));
	Log_TestInfo("Кэп брига Queen вышел из: " + sld.city + " и направился в: " + sld.quest.targetShore);
	//==> на карту
	sld.mapEnc.type = "trade";
	//выбор типа кораблика на карте
	sld.mapEnc.worldMapShip = "quest_ship";
...
void LSC_admiralTakeAll()
{
	...
	AddSimpleRumourCityTip("Вы знаете, муж Элис Тейлор попал в тюрьму, адмирал посадил его за дебош у него в резиденции. И теперь бедняга Элис просит помоч ей вызволить супруга.", "LostShipsCity", 10, 1, "LSC", "LSC_rumourElisHusband");
...
void NavalExitVelasco(string qName)
{
	...
	AddSimpleRumourCityTip("Хм, вы, конечно, герой и все такое, но, может быть, не стоило так радиально с 'нарвалами' разбираться?..", "LostShipsCity", 10, 1, "LSC", "");
